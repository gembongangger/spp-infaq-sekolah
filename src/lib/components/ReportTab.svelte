<script lang="ts">
	...
	// Calculate totals
	const totalInfaq = $derived(filteredTransactions
		.filter(t => t.jenis === 'masuk' && t.kategori === 'infaq')
		.reduce((sum, t) => sum + t.jumlah, 0));

	const totalJariyah = $derived(filteredTransactions
		.filter(t => t.jenis === 'masuk' && t.kategori === 'jariyah')
		.reduce((sum, t) => sum + t.jumlah, 0));

	const totalKeluar = $derived(filteredTransactions
	...
	async function exportToExcel() {
		...
		data.push({
			...
			Pengirim: '',
			Kelas: '',
			Jumlah: filteredTransactions.reduce((sum, t) => sum + (t.jenis === 'masuk' ? t.jumlah : -t.jumlah), 0)
		} as any);
...
<div class="fade-in">
	<div class="rounded-2xl p-5 {cardBg} {cardBorder} mb-6">
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
			...
			<div>
				...
				<select
					value={filterNama}
					onchange={(e) => onFilterNamaChange(e.currentTarget.value)}
					class="w-full px-3 py-2.5 rounded-xl text-sm {inputBg} {inputBorder} {inputText}"
				>
			...
			<div>
				...
				<select
					value={filterKelas}
					onchange={(e) => onFilterKelasChange(e.currentTarget.value)}
					class="w-full px-3 py-2.5 rounded-xl text-sm {inputBg} {inputBorder} {inputText}"
				>
			...
			<div>
				...
				<select
					value={filterMetode}
					onchange={(e) => onFilterMetodeChange(e.currentTarget.value)}
					class="w-full px-3 py-2.5 rounded-xl text-sm {inputBg} {inputBorder} {inputText}"
				>
	...
	<div class="rounded-2xl p-5 {cardBg} {cardBorder}" id="report-preview-table">
		<h3 class="text-sm font-semibold mb-4 {textSecondary}">
			📊 Preview Laporan ({filteredTransactions.length} transaksi)
		</h3>
		...
		{:else}
			<div class="text-center py-12">
				<p class="{textMuted}">Tidak ada data untuk ditampilkan</p>
			</div>
		{/if}
	</div>
</div>
